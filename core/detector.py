...
class DriverMonitor:
        ...
        def _init_detector(self):
            ...
            # your existing MediaPipe initialization here
                ...
                self.backend = "none"
                print("[ERROR] No face detection backend available.")
        ...
        def _mar(self, pts: np.ndarray, mouth_idx: List[int]) -> float:
            """Mouth Aspect Ratio for yawn detection using dlib 68 landmarks."""
            A = dist.euclidean(pts[51], pts[59])
            B = dist.euclidean(pts[53], pts[57])
            C = dist.euclidean(pts[62], pts[66])
